...
const PORTAL_SELECTED_CLASSES = ['bg-gray-300', 'text-gray-900', 'border', 'border-gray-400'];

function applyTagStylesPortal(selectedTags) {
    document.querySelectorAll('.tag-button').forEach(btn => {
        ...
        btn.className = btn.className
            .split(' ')
            .filter(c => !PORTAL_SELECTED_CLASSES.includes(c))
            .join(' ');
        originalStyles.split(' ').forEach(cls => {
            if (cls) btn.classList.add(cls);
        });
        if (selectedTags.includes(tag)) {
            PORTAL_SELECTED_CLASSES.forEach(cls => btn.classList.add(cls));
        }
    });
}
...
window.toggleTag = function(element, tag) {
    ...
    applyTagStylesPortal(selectedTags);
    console.log('Filtro de tags (portal) atualizado:', selectedTags);
    fetchPosts(selectedTags);
};

function fetchPosts(tags) {
    fetch('{{ route('portal.filter') }}', {
        method: 'POST',
        headers: {
    ...
    .then(response => {
        if (!response.ok) {
            console.error('Erro na resposta do filtro (portal):', response.status, response.statusText);
        }
        return response.json();
    })
    .then(data => {
        console.log('Posts filtrados (portal) recebidos:', data);
        if (data && data.posts) {

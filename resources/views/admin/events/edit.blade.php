    ...
    <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden mb-6">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    @if($event->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $event->image) }}" alt="{{ $event->title }}" class="h-32 w-auto object-cover rounded">
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden mb-6">
            ...
            <div id="speakers-container" class="p-6 space-y-6">
                <div class="text-sm text-gray-500 text-center py-4" id="no-speakers-message" style="{{ $event->speakers->count() > 0 ? 'display: none;' : '' }}">
                    ...
                    @foreach($event->speakers as $index => $speaker)
                        <div class="speaker-card bg-gray-50 rounded-lg p-4 relative">
                            ...
                            <div class="mt-4">
                                ...
                                @if($speaker->photo)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/' . $speaker->photo) }}" alt="{{ $speaker->name }}" class="h-20 w-20 object-cover rounded-full">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let speakerCount = {{ $event->speakers->count() }};
        ...
        function addSpeaker() {
            ...
            // Inicializar os eventos de remoção para palestrantes existentes
            document.querySelectorAll('.speaker-card .remove-speaker').forEach(button => {
